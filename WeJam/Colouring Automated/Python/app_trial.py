...
#beats_per_bar = input("Enter the number of beats in a bar: ")
...
font = ImageFont.truetype("./Nootype - Radikal.otf", 80)  
coord = detect_bar(input_directory_path)
start_dict= detect_X(input_directory_path)
#start_df['Length'] = 0
#start_df = start_df.sort_values(by=['Page', 'Start'])
...
#    #print("instrument:", instrument_id.program)
    for note in instrument_id.notes:
        df['Bar'][i] = int((bps * note.start)/beats_per_bar) +1
        page = int(((bps * note.start)/beats_per_bar)/2 + 1) 
        df["Page"][i] = page
        df["End"][i] = note.end
        ...
        dur = df['Duration'][i]
        df['Bar_start'][i] = int((bps * note.start)/beats_per_bar)%2 + 1
...
for k in range(len(df)): 
    ...
    start_div = int( start_div  * div_bar)
...
#                    try:
        ...
        # piano
        if instrument == "piano" or instrument == "Piano":
            if pitch > 20 and pitch < 32 or (pitch == 32 and flat == False) or (pitch == 20 and flat == True):
                colour = magenta
            elif pitch > 32 and pitch < 44 or (pitch == 44 and flat == False) or (pitch == 32 and flat == True):
                colour = magenta
                ...
                colour = orange
        # font 
        font = ImageFont.truetype("./Nootype - Radikal.otf", 40)
        try:
        ...
        if note.endswith('\u266D'):
            ...
            font  = ImageFont.truetype("C:/Users/arvin/Documents/WeJam/Band Name Generator/final_app/Musisync-KVLZ.ttf", 70)
